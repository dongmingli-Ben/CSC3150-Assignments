...
__device__ void user_program(FileSystem *fs, uchar *input, uchar *output) {
	...
	/////////////// Test Case 5  ///////////////
	// u32 fp;
	// fp = fs_open(fs, "c.txt", G_WRITE);
	// fs_write(fs, input, 10, fp);
	...
	// fs_write(fs, input, 1024*1024 - 32, fp);
	// fs_gsys(fs, LS_S);
	// fs_read(fs, output, 1024*1024 - 32, fp);
	// fs_write(fs, input, 1023*1024, fp);
	// fp = fs_open(fs, "b.txt", G_WRITE);
	// fs_write(fs, input, 10, fp);
	// fs_gsys(fs, LS_S);
	// fp = fs_open(fs, "b.txt", G_WRITE);
	// fs_write(fs, input, 0, fp);
	// fs_gsys(fs, LS_S);
}
